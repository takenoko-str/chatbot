...
def read_data_into_buckets(enc_path, dec_path, buckets):
    ...
    with tf.gfile.GFile(
            enc_path, mode='r') as ef, tf.gfile.GFile(
                dec_path, mode='r') as df:
        ...
        while tweet and reply:
            ...
            for bucket_id, (source_size, target_size) in enumerate(buckets):
                # Find bucket to put this conversation based on tweet and reply
                # length
...
def create_or_restore_model(session, buckets, forward_only, beam_search,
    ...
    model = seq2seq_model.Seq2SeqModel(
        source_vocab_size=data_processor.MAX_ENC_VOCABULARY,
...
def next_random_bucket_id(buckets_scale):
    ...
    bucket_id = min(
        [i for i in range(len(buckets_scale)) if buckets_scale[i] > n])
    return bucket_id
...
def train():
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        train_bucket_sizes = [
            len(train_set[b]) for b in range(len(data_processor.buckets))
        ]
        ...
        while True:
            ...
            # Train!
            _, average_perplexity, _ = model.step(
                sess,
                encoder_inputs,
                decoder_inputs,
                target_weights,
            ...
            perplexity = math.exp(
                average_perplexity) if average_perplexity < 300 else float(
                    'inf')
            print("global step %d learning rate %.4f perplexity "
                  "%.2f" % (model.global_step.eval(),
                            model.learning_rate.eval(), perplexity))

            # Decrease learning rate if no improvement was seen over last 3
            # times.
            if len(previous_perplexities) > 2 and perplexity > max(
                    previous_perplexities[-3:]):
