    ...
    def on_status(self, status):
        ...
        if status.in_reply_to_status_id_str is not None:
            ...
            if len(self.statuses) == 100:
                ...
                # Note: response is smaller than request if some tweets were
                # private
                source_statuses = self.api.statuses_lookup(
                    in_reply_to_ids, trim_user=True)

                # construct dictionary of source id to target text
                target_dictionary = {
                    status.in_reply_to_status_id_str: status.text
                    for status in self.statuses
                }
...
def main():
    ...
    while True:
        try:
            ...
            api = tweepy.API(auth)
            reply_stream_listener = ReplyStreamListener(
                api, args.target_file, args.source_file)
            reply_stream = tweepy.Stream(
                auth=api.auth, listener=reply_stream_listener)
