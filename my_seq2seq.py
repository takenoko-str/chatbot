...
def _extract_beam_search(embedding,
                         beam_size,
                         num_symbols,
                         embedding_size,
                         output_projection=None,
            ...
            outputs.append(
                tf.argmax(
                    tf.nn.xw_plus_b(output, output_projection[0],
                                    output_projection[1]),
                    axis=1))
    return outputs, state, tf.reshape(
        tf.concat(axis=0, values=beam_path), [-1, beam_size]), tf.reshape(
...
def embedding_rnn_decoder(decoder_inputs,
  ...
  """
    if output_projection is not None:
        proj_weights = tf.convert_to_tensor(
            output_projection[0], dtype=tf.float32)
  ...
  """
    with tf.variable_scope(scope or "embedding_attention_seq2seq"):
        ...
        encoder_outputs, encoder_state = tf.contrib.rnn.static_rnn(
            ...
            dtype=dtype)
        print('####### embedding_attention_seq2seq scope: {}'.format(
            encoder_cell))
        print("Symbols")
        ...
        # on.
        top_states = [
            tf.reshape(e, [-1, 1, cell_1.output_size]) for e in encoder_outputs
        ]
    ...
    with tf.name_scope(name, "model_with_buckets", all_inputs):
        for j, bucket in enumerate(buckets):
            with tf.variable_scope(
                    tf.get_variable_scope(), reuse=True if j > 0 else None):
